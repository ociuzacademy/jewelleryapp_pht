...
def sales_report(request):
    ...
    print(f"All Product Bookings: {ProductBookings.objects.count()}")
    print(f"All Cart Bookings: {Cart.objects.count()}")

    # ✅ Filter ProductBookings & Cart based on date range
    product_bookings = ProductBookings.objects.filter(booking_date__range=[start_date, end_date])
    cart_bookings = Cart.objects.filter(created_at__range=[start_date, end_date])

    print(f"Product Bookings Count: {product_bookings.count()}")
    print(f"Cart Bookings Count: {cart_bookings.count()}")

    # ✅ Sales Summary
    total_sales = product_bookings.aggregate(total_sales=Sum("total_price"))["total_sales"] or 0
    total_advance_fee = product_bookings.aggregate(total_advance=Sum("advance_fee"))["total_advance"] or 0
    total_orders = product_bookings.count()

    # ✅ Status Breakdown
    status_summary = (
        product_bookings.values("status")
        .annotate(total_count=Count("id"), total_amount=Sum("total_price"))
        .order_by("status")
    )

    # ✅ Employee Assignment Summary
    employee_summary = (
        product_bookings.values("assigned_employee__name")
        .annotate(total_count=Count("id"), total_amount=Sum("total_price"))
        .order_by("-total_count")
    )

    # ✅ Cart Sales Summary
    cart_sales = cart_bookings.aggregate(cart_sales=Sum("total_price"))["cart_sales"] or 0
    cart_advance_fee = cart_bookings.aggregate(cart_advance=Sum("advance_fee"))["cart_advance"] or 0
    cart_orders = cart_bookings.count()

    # ✅ Final Context to Pass to Template
    context = {
        "start_date": start_date.strftime("%Y-%m-%d"),
        "end_date": end_date.strftime("%Y-%m-%d"),
        "total_sales": total_sales + cart_sales,
        "total_advance_fee": total_advance_fee + cart_advance_fee,
        "total_orders": total_orders + cart_orders,
